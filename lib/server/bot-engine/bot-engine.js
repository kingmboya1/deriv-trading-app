...
const deriv_connection_1 = require("./deriv-connection");
/**
...
class BotEngine {
     ...
     */
    constructor(userId, config, derivToken) {
        this.derivConnection = null;
    ...
    async startSession() {
        ...
        const connection = new deriv_connection_1.DerivConnection(this.derivToken);
        ...
        const balanceResponse = await connection.request({ balance: 1 });
        ...
        connection.subscribe({ proposal_open_contract: 1, subscribe: 1 }, (data) => {
    ...
    async placeTrade(stake) {
        ...
        let payout;
        try {
            const proposalResponse = await this.derivConnection.request(proposalPayload);
            // Check for API error
        ...
        let buyPrice;
        try {
            const buyResponse = await this.derivConnection.request({
                buy: proposalId,
                price: askPrice,
            });
            // Check for API error
     ...
     */
    handleContractSettlement(contractId, status, profit) {
