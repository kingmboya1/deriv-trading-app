...
/**
 * SidebarNav — left sidebar with icon + label nav sections.
 * Sections map directly to panels that exist today. "Auto Trade" is listed
 * as coming-soon so the nav slot is reserved without being selectable.
 */
...
export default function SidebarNav({ active, onChange }: SidebarNavProps) {
  return (
    <aside className="flex h-full w-56 flex-none flex-col border-r border-hairline bg-surface">
      {/* Logo row */}
            ...
            return (
              <li key={item.id}>
                <button
                  ...
                  disabled={isDisabled}
                  onClick={() => {
                    if (!isDisabled && item.id !== "autotrade") {
                ...
                >
                  {/* Icon — accent when active, muted otherwise */}
